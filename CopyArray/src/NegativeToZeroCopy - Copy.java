...
public class NegativeToZeroCopy {
   public static void main(String[] args) {
      ...
      for (i = 0; i < userVals.length; ++i) {    // populates the values
         System.out.print("Value: ");
         userVals[i] = scnr.nextInt();
      }

      // Convert nums to newNums
      for (i = 0; i < userVals.length; ++i) {     //converts from old arrays to new arays
         copiedVals[i] = userVals[i];
      }

      // Convert negatives to 0
      for (i = 0; i < copiedVals.length; ++i) {  // takes negative values and assignes with zero
         if (copiedVals[i] < 0) {
            copiedVals[i] = 0;
         }
      }
